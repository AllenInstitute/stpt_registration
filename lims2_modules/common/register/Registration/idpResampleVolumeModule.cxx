...
int main( int argc, char *argv[] )
{
  ...
  std::string modelDirectory = "/projects/0310/vol1/informatics/model";
  int threads = 1;
  double redScale = 0.34;
  double greenScale = 0.66;
  double blueScale = 0.0;
  int imageBackground = 255;
  bool invertIntensity = true;
  bool generateMask = true;
  bool invertDarkFieldImages = true;
  ...
  arg.AddArgument( "--imageBackground", argT::SPACE_ARGUMENT, &imageBackground, "image background" );
  arg.AddArgument( "--invertIntensity", argT::SPACE_ARGUMENT, &invertIntensity, "invert intensity" );
  arg.AddArgument( "--generateMask", argT::SPACE_ARGUMENT, &generateMask, "generate mask" );
  arg.AddArgument( "--invertDarkFieldImages", argT::SPACE_ARGUMENT, &invertDarkFieldImages, "invert dark field images" );
  ...
  std::cout << "imageBackground: " << imageBackground << std::endl;
  std::cout << "invertIntensity: " << invertIntensity << std::endl;
  std::cout << "generateMask:" << generateMask << std::endl;
  arg.DeleteRemainingArguments( lastArgc, &lastArgv );
  }
  ...
  try
    ...
    dispatcher->SetImageBackground( imageBackground );
    dispatcher->SetInvertIntensity( invertIntensity );

    dispatcher->GetUtility()->SetInvertDarkFieldImages( invertDarkFieldImages );

    // Read in transforms
    dispatcher->LoadTransformsFromXML( transformXMLFile.c_str() );

    // populate volume
    dispatcher->PopulateVolume(redScale,greenScale,blueScale);
    ...
    Affine3DTransformType::Pointer vtoa = dispatcher->GetVolumeToAtlasTransform();
    itk::idp::ReadImage<VolumeType>( path.c_str(), grid50 );

    VolumeType::Pointer output;
    VolumeType::Pointer volume = dispatcher->GetVolume();
    itk::idp::ResampleImage<VolumeType>(  grid50,
                                          volume, vtoa, output, 0, "Linear" );
    path = outputDirectory.c_str();
    path += "/resampledGrid50.mhd";
    itk::idp::WriteImage<VolumeType>( path.c_str(), output );

    // resample atlas
    dispatcher->ReadAtlasData();
    dispatcher->ComposeWithGridToAtlasTransform();
    dispatcher->ResampleAtlas();
    dispatcher->WriteResampledAtlas( outputDirectory.c_str() );
